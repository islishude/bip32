...
func TestBIP32OfficialVectors(t *testing.T) {
	...
	for vectorIndex, vector := range vectors {
		...
		if err != nil {
			t.Fatalf("decode vector %d seed: %v", vectorIndex+1, err)
		}
		root, err := NewMasterKey(seed, Mainnet)
		if err != nil {
			t.Fatalf("vector %d master: %v", vectorIndex+1, err)
		}
		for _, node := range vector.nodes {
			t.Run(node.path, func(t *testing.T) {
				key, err := root.DerivePath(node.path)
				if err != nil {
					t.Fatalf("DerivePath: %v", err)
				}
				gotXPrv, err := key.Encode()
				if err != nil {
					t.Fatalf("Encode xprv: %v", err)
				}
				if gotXPrv != node.xprv {
					t.Fatalf("xprv = %s, want %s", gotXPrv, node.xprv)
				}
				xpub, err := key.XPub()
				if err != nil {
					t.Fatalf("XPub: %v", err)
				}
				gotXPub, err := xpub.Encode()
				if err != nil {
					t.Fatalf("Encode xpub: %v", err)
				}
				if gotXPub != node.xpub {
					t.Fatalf("xpub = %s, want %s", gotXPub, node.xpub)
				}
				assertEncodedRoundTrip(t, node.xprv, node.xpub)
			})
		}
	}
}

func assertEncodedRoundTrip(t *testing.T, xprv, xpub string) {
	t.Helper()
	private, err := ParseXPrv(xprv)
	if err != nil {
		t.Fatalf("ParseXPrv: %v", err)
	}
	privateEncoded, err := private.Encode()
	if err != nil || privateEncoded != xprv {
		t.Fatalf("xprv round trip = %q, %v", privateEncoded, err)
	}
	public, err := ParseXPub(xpub)
	if err != nil {
		t.Fatalf("ParseXPub: %v", err)
	}
	publicEncoded, err := public.Encode()
	if err != nil || publicEncoded != xpub {
		t.Fatalf("xpub round trip = %q, %v", publicEncoded, err)
	}
}

func TestBIP32OfficialInvalidExtendedKeys(t *testing.T) {
	invalid := []string{
	...
	for _, encoded := range invalid {
		t.Run(encoded[:8], func(t *testing.T) {
			var err error
			switch {
			case strings.HasPrefix(encoded, "xprv"):
				_, err = ParseXPrv(encoded)
			case strings.HasPrefix(encoded, "xpub"):
				_, err = ParseXPub(encoded)
			default:
				_, prvErr := ParseXPrv(encoded)
				_, pubErr := ParseXPub(encoded)
				if prvErr == nil || pubErr == nil {
					t.Fatalf("unknown-version key accepted: prv=%v pub=%v", prvErr, pubErr)
				}
				return
			}
			if err == nil {
				t.Fatal("invalid extended key accepted")
			}
		})
	}
}
